...
export class SystemManageService implements OnModuleInit {
  ...
  async seedData() {
    ...
    if (menuCount === 0) {
      ...
      const menus = [
        { name: '仪表盘', route: 'home', order: 1, icon: 'mdi:monitor-dashboard' },
        { name: '商品管理', route: 'function_product', order: 2, icon: 'icon-park-outline:commodity' },
        { name: '订单管理', route: 'function_order', order: 3, icon: 'icon-park-outline:transaction-order' },
        { name: '系统管理', route: 'function_system', order: 4, icon: 'icon-park-outline:system' },
      ];
      ...
      const superAdmin = await this.roleRepository.findOne({ where: { code: 'R_SUPER' } });
      if (superAdmin) {
  ...
  async getRoleList(page = 1, pageSize = 10, keyword?: string, status?: string) {
  ...
  async updateUserRoles(userId: string, roleCodes: string[]) {
    ...
    const roles = await this.roleRepository.find({ where: { code: In(roleCodes) } });
    user.roles = roles;
  ...
  async getUserList(page = 1, pageSize = 10, keyword?: string) {
    ...
    if (keyword) {
      qb.andWhere('(user.phone LIKE :keyword OR user.nickname LIKE :keyword)', { keyword: `%${keyword}%` });
    }
